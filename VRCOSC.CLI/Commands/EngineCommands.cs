...
internal static class EngineCommands
{
    ...
    public sealed class StartCommand : AsyncCommand<StartCommand.Settings>
    {
        ...
        {
            var appManager = AppManager.GetInstance();
            var isForce = settings.Force || string.Equals(context.Name, "force-start", StringComparison.OrdinalIgnoreCase);

            await AnsiConsole.Status()
                .Spinner(Spinner.Known.Dots)
                .SpinnerStyle(new Style(Color.Cyan1))
                .StartAsync(isForce ? "Force starting VRCOSC engine modules..." : "Requesting VRCOSC auto-connection startup...", async ctx =>
                {
                    if (isForce)
                    {
